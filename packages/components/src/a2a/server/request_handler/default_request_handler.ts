...
export class DefaultRequestHandler implements A2ARequestHandler {
    ...
    private async _createRequestContext(incomingMessage: Message, taskId: string, isStream: boolean): Promise<RequestContext> {
    ...
    async cancelTask(params: TaskIdParams): Promise<Task> {
        ...
        if (eventBus) {
            ...
            if (!task.status.message) {
                throw A2AError.internalError(`Task ${params.id} does not have a status message.`)
            }
            if (!task.status.message) {
                throw A2AError.internalError(`Task ${params.id} does not have a status message.`)
            }
    ...
    async getTaskPushNotificationConfig(params: TaskIdParams | GetTaskPushNotificationConfigParams): Promise<TaskPushNotificationConfig> {
        if (!this.agentCard.capabilities.pushNotifications) {
            throw A2AError.pushNotificationNotSupported()
        }
        const task = await this.taskStore.load(params.id)
