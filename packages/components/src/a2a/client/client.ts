...
export class A2AClient {
     ...
     */
    private async _postRpcRequest<TParams, TResponse extends JSONRPCResponse>(method: string, params: TParams): Promise<TResponse> {
        ...
        if (!httpResponse.ok) {
            ...
            try {
                ...
                if (errorJson.jsonrpc && errorJson.error) {
                    ...
                    throw new Error(
                        `RPC error for ${method}: ${errorJson.error.message} (Code: ${errorJson.error.code}, HTTP Status: ${
                            httpResponse.status
    ...
    public async *sendMessageStream(params: MessageSendParams): AsyncGenerator<A2AStreamEventData, void, undefined> {
        ...
        if (!response.ok) {
            ...
            try {
                ...
                throw new Error(
                    `HTTP error establishing stream for message/stream: ${response.status} ${response.statusText}. Response: ${
                        errorBody || '(empty)'
                    }`
                )
            }
    ...
    public async getTaskPushNotificationConfig(params: TaskIdParams): Promise<GetTaskPushNotificationConfigResponse> {
        ...
        return this._postRpcRequest<TaskIdParams, GetTaskPushNotificationConfigResponse>('tasks/pushNotificationConfig/get', params)
    }
    ...
    public async *resubscribeTask(params: TaskIdParams): AsyncGenerator<A2AStreamEventData, void, undefined> {
        ...
        if (!response.ok) {
            ...
            try {
                ...
                if (e.message.startsWith('HTTP error establishing stream')) throw e
                throw new Error(
                    `HTTP error establishing stream for tasks/resubscribe: ${response.status} ${response.statusText}. Response: ${
                        errorBody || '(empty)'
                    }`
                )
            }
     ...
     */
    private _processSseEventData<TStreamItem>(jsonData: string, originalRequestId: number | string | null): TStreamItem {
        if (!jsonData.trim()) {
