...
class Milvus_VectorStores implements INode {
    ...
    async init(nodeData: INodeData, _: string, options: ICommonObject): Promise<any> {
        ...
        if (isFileUploadEnabled && options.chatId) {
            if (milvusFilter) milvusFilter += ` OR ${THUB_CHATID} == "${options.chatId}" OR NOT EXISTS(${THUB_CHATID})`
            else milvusFilter = `${THUB_CHATID} == "${options.chatId}" OR NOT EXISTS(${THUB_CHATID})`
        }
