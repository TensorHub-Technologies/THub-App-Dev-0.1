...
class A2AExpressApp {
     ...
     */
    setupRoutes(app, baseUrl = '', middlewares, agentCardPath = constants_js_1.AGENT_CARD_PATH) {
        const router = express_1.default.Router();
        router.use(express_1.default.json(), ...(middlewares ?? []));
            ...
            try {
                ...
                if (typeof rpcResponseOrStream?.[Symbol.asyncIterator] === 'function') {
                    ...
                    try {
                        ...
                        const a2aError = streamError instanceof error_js_1.A2AError
                            ? streamError
                            : error_js_1.A2AError.internalError(streamError.message || 'Streaming error.');
                        const errorResponse = {
                            jsonrpc: '2.0',
