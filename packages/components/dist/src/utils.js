...
exports.THUB_CHATID = 'thub_chatId';
...
const getEncryptionKeyFilePath = () => {
    const checkPaths = [
        ...
        path.join(__dirname, '..', '..', '..', '..', '..', 'encryption.key'),
        path.join(__dirname, '..', '..', '..', '..', '..', 'server', 'encryption.key'),
        path.join((0, exports.getUserHome)(), '.thub', 'encryption.key')
    ];
    for (const checkPath of checkPaths) {
        if (fs.existsSync(checkPath)) {
            return checkPath;
...
const jsonEscapeCharacters = [
    { escape: '"', value: 'THUB_DOUBLE_QUOTE' },
    { escape: '\n', value: 'THUB_NEWLINE' },
    { escape: '\b', value: 'THUB_BACKSPACE' },
    { escape: '\f', value: 'THUB_FORM_FEED' },
    { escape: '\r', value: 'THUB_CARRIAGE_RETURN' },
    { escape: '\t', value: 'THUB_TAB' },
    { escape: '\\', value: 'THUB_BACKSLASH' }
];
...
const getVars = async (appDataSource, databaseEntities, nodeData, options) => {
    ...
    const variables = (await appDataSource.getRepository(databaseEntities['Variable']).findBy({ tenantId: (0, typeorm_1.Equal)(options.tenantId) })) ??
        [];
...
async function parseWithTypeConversion(schema, arg, maxDepth = 10) {
    ...
    try {
        ...
        if (zod_1.z.ZodError && e instanceof zod_1.z.ZodError) {
            const zodError = e;
...
const configureStructuredOutput = (llmNodeInstance, structuredOutput) => {
    try {
        ...
        const structuredOutputSchema = zod_1.z.object(zodObj);
        // @ts-ignore
