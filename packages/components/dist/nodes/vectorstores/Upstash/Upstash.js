...
class Upstash_VectorStores {
            ...
            for (let i = 0; i < flattenDocs.length; i += 1) {
                if (flattenDocs[i] && flattenDocs[i].pageContent) {
                    if (isFileUploadEnabled && options.chatId) {
                        flattenDocs[i].metadata = { ...flattenDocs[i].metadata, [utils_1.THUB_CHATID]: options.chatId };
                    }
    ...
    async init(nodeData, _, options) {
        ...
        if (isFileUploadEnabled && options.chatId) {
            if (upstashMetadataFilter)
                obj.filter += ` OR ${utils_1.THUB_CHATID} = "${options.chatId}" OR HAS NOT FIELD ${utils_1.THUB_CHATID}`;
            else
                obj.filter = `${utils_1.THUB_CHATID} = "${options.chatId}" OR HAS NOT FIELD ${utils_1.THUB_CHATID}`;
        }
        const vectorStore = await upstash_1.UpstashVectorStore.fromExistingIndex(embeddings, obj);
