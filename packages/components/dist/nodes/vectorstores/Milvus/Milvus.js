...
class Milvus_VectorStores {
    ...
    async init(nodeData, _, options) {
        ...
        if (isFileUploadEnabled && options.chatId) {
            if (milvusFilter)
                milvusFilter += ` OR ${utils_1.THUB_CHATID} == "${options.chatId}" OR NOT EXISTS(${utils_1.THUB_CHATID})`;
            else
                milvusFilter = `${utils_1.THUB_CHATID} == "${options.chatId}" OR NOT EXISTS(${utils_1.THUB_CHATID})`;
        }
