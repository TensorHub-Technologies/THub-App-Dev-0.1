...
class TypeORMDriver extends Base_1.VectorStoreDriver {
    ...
    async adaptInstance(instance, metadataFilters) {
        ...
        */
        instance.similaritySearchVectorWithScore = async (query, k, filter) => {
            return await TypeORMDriver.similaritySearchVectorWithScore(query, k, tableName, await this.getPostgresConnectionOptions(), filter ?? metadataFilters, this.computedOperatorString);
        };
    ...
    static similaritySearchVectorWithScore = async (query, k, tableName, postgresConnectionOptions, filter, distanceOperator = '<=>') => {
        const embeddingString = `[${query.join(',')}]`;
        let chatflowOr = '';
        const { [src_1.THUB_CHATID]: chatId, ...restFilters } = filter || {};
        ...
        // https://github.com/THub/THub/pull/3367#discussion_r1804229295
        if (chatId) {
            parameters.push({ [src_1.THUB_CHATID]: chatId });
            chatflowOr = `OR metadata @> $${parameters.length}`;
        }
        const queryString = `
            SELECT *, embedding ${distanceOperator} $1 as "_distance"
            FROM ${tableName}
            WHERE ((metadata @> $2) AND NOT (metadata ? '${src_1.THUB_CHATID}')) ${chatflowOr}
