            ...
            for (const item of message.content) {
                ...
                if (item.type === 'stored-file' && item.name && item.mime.startsWith('image/')) {
                    ...
                    try {
                        ...
                        const contents = await (0, storageUtils_1.getFileFromStorage)(item.name, options.chatflowid, options.chatId);
            ...
            try {
                ...
                for (const upload of uploads) {
                    if (upload.type === 'stored-file' && upload.mime.startsWith('image/')) {
                        const fileData = await (0, storageUtils_1.getFileFromStorage)(upload.name, options.chatflowid, options.chatId);
                        // as the image is stored in the server, read the file and convert it to base64
