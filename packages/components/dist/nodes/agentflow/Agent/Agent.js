...
class Agent_Agentflow {
    ...
    async run(nodeData, input, options) {
        ...
        try {
            ...
            for (const tool of tools) {
                ...
                const newNodeData = {
                    ...nodeData,
                    credential: toolConfig['THUB_CREDENTIAL_ID'],
                    inputs: {
                        ...nodeData.inputs,
                        ...toolConfig
                    }
            ...
            if (knowledgeBasesForVSEmbeddings && knowledgeBasesForVSEmbeddings.length > 0) {
                for (const knowledgeBase of knowledgeBasesForVSEmbeddings) {
                    ...
                    const newEmbeddingNodeData = {
                        ...nodeData,
                        credential: selectedEmbeddingModelConfig['THUB_CREDENTIAL_ID'],
                        inputs: {
                            ...nodeData.inputs,
                    ...
                    const newVSNodeData = {
                        ...nodeData,
                        credential: selectedVectorStoreConfig['THUB_CREDENTIAL_ID'],
                        inputs: {
                            ...nodeData.inputs,
    ...
    async saveBase64Image(outputItem, options) {
        try {
            ...
            const path = await (0, storageUtils_1.addSingleFileToStorage)(mimeType, imageBuffer, fileName, options.chatflowid, options.chatId);
    ...
    async downloadContainerFile(containerId, fileId, filename, modelNodeData, options) {
        try {
            ...
            const path = await (0, storageUtils_1.addSingleFileToStorage)(mimeType, dataBuffer, filename, options.chatflowid, options.chatId);
