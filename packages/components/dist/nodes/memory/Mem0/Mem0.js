...
const initializeMem0 = async (nodeData, input, options) => {
    ...
    const useTHubChatId = nodeData.inputs?.useTHubChatId;
...
class Mem0MemoryExtended extends community_1.Mem0Memory {
    ...
    // Selects Mem0 user_id based on toggle state (THub chat ID or input field)
    getEffectiveUserId(overrideUserId) {
        ...
        if (this.useTHubChatId) {
            if (overrideUserId) {
                ...
                throw new Error('Mem0: "Use THub Chat ID" is ON, but no runtime chat ID (overrideUserId) was provided.');
            }
        }
        else {
