...
const getAllVariables = async (tenantId: any) => {
    try {
        const appServer = getRunningExpressApp()
        const dbResponse = await appServer.AppDataSource.getRepository(Variable).findBy({ tenantId })
        return dbResponse
    } catch (error) {
        throw new InternalTHubError(
            StatusCodes.INTERNAL_SERVER_ERROR,
    ...
    try {
        for (const data of newVariables) {
            if (data.id && !validate(data.id)) {
                throw new InternalTHubError(StatusCodes.PRECONDITION_FAILED, `Error: importVariables - invalid id!`)
            }
        }
        ...
        throw new InternalTHubError(StatusCodes.INTERNAL_SERVER_ERROR, `Error: variableService.importVariables - ${getErrorMessage(error)}`)
    }
}
