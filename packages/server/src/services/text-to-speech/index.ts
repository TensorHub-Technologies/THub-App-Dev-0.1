...
const getVoicesForProvider = async (provider: string, credentialId?: string): Promise<any[]> => {
    try {
        if (!credentialId) {
            throw new InternalTHubError(StatusCodes.BAD_REQUEST, 'Credential ID required for this provider')
        }
        ...
        throw new InternalTHubError(StatusCodes.INTERNAL_SERVER_ERROR, `Error: textToSpeechService.getVoices - ${getErrorMessage(error)}`)
    }
}
