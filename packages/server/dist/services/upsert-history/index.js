...
const getAllUpsertHistory = async (sortOrder, chatflowid, startDate, endDate) => {
    try {
        ...
        throw new internalTHubError_1.InternalTHubError(http_status_codes_1.StatusCodes.INTERNAL_SERVER_ERROR, `Error: upsertHistoryServices.getAllUpsertHistory - ${(0, utils_1.getErrorMessage)(error)}`);
    }
};
const patchDeleteUpsertHistory = async (ids = []) => {
    try {
        ...
        throw new internalTHubError_1.InternalTHubError(http_status_codes_1.StatusCodes.INTERNAL_SERVER_ERROR, `Error: upsertHistoryServices.patchDeleteUpsertHistory - ${(0, utils_1.getErrorMessage)(error)}`);
    }
};
exports.default = {
    getAllUpsertHistory,
