...
const checkFlowValidation = async (flowId) => {
    try {
        ...
        if (!flow) {
            throw new internalTHubError_1.InternalTHubError(http_status_codes_1.StatusCodes.NOT_FOUND, `Error: validationService.checkFlowValidation - flow not found!`);
        }
                    ...
                    if (node.data.inputs[configKey] && node.data.inputs[param.name]) {
                        ...
                        if (componentNodes[componentName] && componentNodes[componentName].inputs) {
                            ...
                            if (componentNodes[componentName].credential && !componentNodes[componentName].credential.optional) {
                                if (!configValue.THUB_CREDENTIAL_ID && !configValue.credential) {
        ...
        throw new internalTHubError_1.InternalTHubError(http_status_codes_1.StatusCodes.INTERNAL_SERVER_ERROR, `Error: validationService.checkFlowValidation - ${(0, utils_1.getErrorMessage)(error)}`);
    }
};
exports.default = {
    checkFlowValidation
