...
const getEnterpriseInquiryRecipient = () => String(process.env.ENTERPRISE_MAIL_TO ||
...
const resolvePlanConfig = (planId) => {
    ...
    if (!planConfig) {
        throw new internalTHubError_1.InternalTHubError(http_status_codes_1.StatusCodes.BAD_REQUEST, 'Invalid plan ID');
    }
    return planConfig;
};
const getSubscriptionDates = (duration) => {
...
const activateFreeSubscription = async (body) => {
    try {
        ...
        if (!customerEmail || !userId) {
            throw new internalTHubError_1.InternalTHubError(http_status_codes_1.StatusCodes.BAD_REQUEST, 'customerEmail and user_id are required');
        }
        ...
        if (normalizeEmail(user.email) !== customerEmail) {
            throw new internalTHubError_1.InternalTHubError(http_status_codes_1.StatusCodes.BAD_REQUEST, 'customerEmail does not match user account email');
        }
        ...
        if (error instanceof internalTHubError_1.InternalTHubError)
            throw error;
        throw new internalTHubError_1.InternalTHubError(http_status_codes_1.StatusCodes.INTERNAL_SERVER_ERROR, `Error: subscriptionService.activateFreeSubscription - ${(0, utils_1.getErrorMessage)(error)}`);
    }
};
const submitEnterpriseMail = async (body) => {
    try {
        ...
        if (!firstName || !lastName || !companyName || !designation || !email || !contactNumber || !description) {
            throw new internalTHubError_1.InternalTHubError(http_status_codes_1.StatusCodes.BAD_REQUEST, 'All enterprise inquiry fields are required');
        }
        const recipient = getEnterpriseInquiryRecipient();
        if (!recipient) {
            throw new internalTHubError_1.InternalTHubError(http_status_codes_1.StatusCodes.SERVICE_UNAVAILABLE, 'Enterprise inquiry recipient email is not configured');
        }
        ...
        if (error instanceof internalTHubError_1.InternalTHubError)
            throw error;
        throw new internalTHubError_1.InternalTHubError(http_status_codes_1.StatusCodes.INTERNAL_SERVER_ERROR, `Error: subscriptionService.submitEnterpriseMail - ${(0, utils_1.getErrorMessage)(error)}`);
    }
};
const createSubscription = async (body) => {
    try {
        ...
        const userId = String(body.user_id || '').trim();
        if (!customerEmail) {
            throw new internalTHubError_1.InternalTHubError(http_status_codes_1.StatusCodes.BAD_REQUEST, 'customerEmail is required');
        }
        if (!planId) {
        ...
        if (!subscription?.id) {
            throw new internalTHubError_1.InternalTHubError(http_status_codes_1.StatusCodes.INTERNAL_SERVER_ERROR, 'Razorpay did not return a subscription ID');
        }
        return {
            id: subscription.id,
        ...
        if (error instanceof internalTHubError_1.InternalTHubError)
            throw error;
        throw new internalTHubError_1.InternalTHubError(http_status_codes_1.StatusCodes.INTERNAL_SERVER_ERROR, `Error: subscriptionService.createSubscription - ${(0, utils_1.getErrorMessage)(error)}`);
    }
};
const validateSubscription = async (body) => {
    try {
        const appServer = (0, getRunningExpressApp_1.getRunningExpressApp)();
        const userRepo = appServer.AppDataSource.getRepository(User_1.User);
        const razorpay = (0, razorpay_1.getRazorpayClient)();
        ...
        if (!subscriptionId || !paymentId || !signature || !planId || !userId) {
            throw new internalTHubError_1.InternalTHubError(http_status_codes_1.StatusCodes.BAD_REQUEST, 'razorpay_subscription_id, razorpay_payment_id, razorpay_signature, planId and user_id are required');
        }
        const planConfig = resolvePlanConfig(planId);
        const signatureIsValid = (0, razorpay_1.verifyRazorpaySubscriptionSignature)(subscriptionId, paymentId, signature);
        if (!signatureIsValid) {
            throw new internalTHubError_1.InternalTHubError(http_status_codes_1.StatusCodes.BAD_REQUEST, 'Payment validation failed');
        }
        ...
        if (razorpaySubscription?.plan_id && razorpaySubscription.plan_id !== planId) {
            throw new internalTHubError_1.InternalTHubError(http_status_codes_1.StatusCodes.BAD_REQUEST, 'Plan mismatch for subscription');
        }
        const noteUserId = String(razorpaySubscription?.notes?.user_id || '').trim();
        if (noteUserId && noteUserId !== userId) {
            throw new internalTHubError_1.InternalTHubError(http_status_codes_1.StatusCodes.FORBIDDEN, 'Subscription does not belong to the provided user');
        }
        const user = await userRepo.findOneBy({ uid: userId });
        if (!user) {
            throw new internalTHubError_1.InternalTHubError(http_status_codes_1.StatusCodes.NOT_FOUND, 'User not found');
        ...
        if (error instanceof internalTHubError_1.InternalTHubError)
            throw error;
        throw new internalTHubError_1.InternalTHubError(http_status_codes_1.StatusCodes.INTERNAL_SERVER_ERROR, `Error: subscriptionService.validateSubscription - ${(0, utils_1.getErrorMessage)(error)}`);
    }
};
exports.default = {
    createSubscription,
