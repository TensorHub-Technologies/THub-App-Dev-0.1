...
const getVoicesForProvider = async (provider, credentialId) => {
    try {
        if (!credentialId) {
            throw new internalTHubError_1.InternalTHubError(http_status_codes_1.StatusCodes.BAD_REQUEST, 'Credential ID required for this provider');
        }
        ...
        return await (0, thub_components_1.getVoices)(provider, credentialId, options);
    }
    catch (error) {
        throw new internalTHubError_1.InternalTHubError(http_status_codes_1.StatusCodes.INTERNAL_SERVER_ERROR, `Error: textToSpeechService.getVoices - ${(0, utils_1.getErrorMessage)(error)}`);
    }
};
exports.default = {
