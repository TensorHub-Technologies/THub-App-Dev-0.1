...
const convertExportInput = (body) => {
    try {
        ...
        throw new internalTHubError_1.InternalTHubError(http_status_codes_1.StatusCodes.INTERNAL_SERVER_ERROR, `Error: exportImportService.convertExportInput - ${(0, utils_1.getErrorMessage)(error)}`);
    }
};
const FileDefaultName = 'ExportData.json';
const exportData = async (exportInput, tenantId) => {
    try {
        let AgentFlow = exportInput.agentflow === true ? (await chatflows_1.default.getAllChatflows('MULTIAGENT', tenantId, 1, 100000)).data : [];
        let AgentFlowV2 = exportInput.agentflowv2 === true ? (await chatflows_1.default.getAllChatflows('AGENTFLOW', tenantId, 1, 100000)).data : [];
        let AssistantCustom = exportInput.assistantCustom === true ? await assistants_1.default.getAllAssistants('CUSTOM', tenantId) : [];
        let AssistantFlow = exportInput.assistantCustom === true ? (await chatflows_1.default.getAllChatflows('ASSISTANT', tenantId, 1, 100000)).data : [];
        let AssistantOpenAI = exportInput.assistantOpenAI === true ? await assistants_1.default.getAllAssistants('OPENAI', tenantId) : [];
        let AssistantAzure = exportInput.assistantAzure === true ? await assistants_1.default.getAllAssistants('AZURE', tenantId) : [];
        let ChatFlow = exportInput.chatflow === true ? (await chatflows_1.default.getAllChatflows('CHATFLOW', tenantId, 1, 100000)).data : [];
        const ownedChatflowIds = new Set([...AgentFlow, ...AgentFlowV2, ...AssistantFlow, ...ChatFlow].map((chatflow) => chatflow.id).filter(Boolean));
        let ChatMessage = exportInput.chat_message === true
            ? (await chat_messages_1.default.getAllMessages()).filter((message) => ownedChatflowIds.has(message.chatflowid))
            : [];
        let ChatMessageFeedback = exportInput.chat_feedback === true
            ? (await chat_messages_1.default.getAllMessagesFeedback()).filter((feedback) => ownedChatflowIds.has(feedback.chatflowid))
            : [];
        let CustomTemplate = exportInput.custom_template === true ? await marketplaces_1.default.getAllCustomTemplates() : [];
        CustomTemplate = CustomTemplate.map((customTemplate) => ({
            ...customTemplate,
            usecases: JSON.stringify(customTemplate.usecases)
        }));
        let DocumentStore = exportInput.document_store === true ? await documentstore_1.default.getAllDocumentStores(-1, -1, tenantId) : [];
        ...
        let DocumentStoreFileChunk = exportInput.document_store === true
            ...
            : [];
        const { data: totalExecutions } = exportInput.execution === true ? await executions_1.default.getAllExecutions({ tenantId, page: 1, limit: 100000 }) : { data: [] };
        let Execution = exportInput.execution === true ? totalExecutions : [];
        let Tool = exportInput.tool === true ? await tools_1.default.getAllTools(tenantId) : [];
        let Variable = exportInput.variable === true ? await variables_1.default.getAllVariables(tenantId) : [];
        return {
            FileDefaultName,
            AgentFlow,
            AgentFlowV2,
        ...
        throw new internalTHubError_1.InternalTHubError(http_status_codes_1.StatusCodes.INTERNAL_SERVER_ERROR, `Error: exportImportService.exportData - ${(0, utils_1.getErrorMessage)(error)}`);
    }
};
async function replaceDuplicateIdsForChatFlow(queryRunner, originalData, chatflows) {
    try {
        ...
        throw new internalTHubError_1.InternalTHubError(http_status_codes_1.StatusCodes.INTERNAL_SERVER_ERROR, `Error: exportImportService.replaceDuplicateIdsForChatflow - ${(0, utils_1.getErrorMessage)(error)}`);
    }
}
async function replaceDuplicateIdsForAssistant(queryRunner, originalData, assistants) {
    try {
        ...
        throw new internalTHubError_1.InternalTHubError(http_status_codes_1.StatusCodes.INTERNAL_SERVER_ERROR, `Error: exportImportService.replaceDuplicateIdsForAssistant - ${(0, utils_1.getErrorMessage)(error)}`);
    }
}
async function replaceDuplicateIdsForChatMessage(queryRunner, originalData, chatMessages) {
    try {
        ...
        const databaseChatflowIds = await (await queryRunner.manager.find(ChatFlow_1.ChatFlow, {
            where: { id: (0, typeorm_1.In)(chatmessageChatflowIds.map((chatmessageChatflowId) => chatmessageChatflowId.id)) }
        ...
        throw new internalTHubError_1.InternalTHubError(http_status_codes_1.StatusCodes.INTERNAL_SERVER_ERROR, `Error: exportImportService.replaceDuplicateIdsForChatMessage - ${(0, utils_1.getErrorMessage)(error)}`);
    }
}
async function replaceExecutionIdForChatMessage(queryRunner, originalData, chatMessages) {
    try {
        ...
        const databaseExecutionIds = await (await queryRunner.manager.find(Execution_1.Execution, {
            where: { id: (0, typeorm_1.In)(chatMessageExecutionIds.map((chatMessageExecutionId) => chatMessageExecutionId.id)) }
        ...
        throw new internalTHubError_1.InternalTHubError(http_status_codes_1.StatusCodes.INTERNAL_SERVER_ERROR, `Error: exportImportService.replaceExecutionIdForChatMessage - ${(0, utils_1.getErrorMessage)(error)}`);
    }
}
async function replaceDuplicateIdsForChatMessageFeedback(queryRunner, originalData, chatMessageFeedbacks) {
    try {
        ...
        const databaseChatflowIds = await (await queryRunner.manager.find(ChatFlow_1.ChatFlow, {
            where: { id: (0, typeorm_1.In)(feedbackChatflowIds.map((feedbackChatflowId) => feedbackChatflowId.id)) }
        })).map((chatflow) => chatflow.id);
        feedbackChatflowIds.forEach((item) => {
            if (databaseChatflowIds.includes(item.id)) {
                item.qty += 1;
        ...
        throw new internalTHubError_1.InternalTHubError(http_status_codes_1.StatusCodes.INTERNAL_SERVER_ERROR, `Error: exportImportService.replaceDuplicateIdsForChatMessageFeedback - ${(0, utils_1.getErrorMessage)(error)}`);
    }
}
async function replaceDuplicateIdsForCustomTemplate(queryRunner, originalData, customTemplates) {
    try {
        ...
        throw new internalTHubError_1.InternalTHubError(http_status_codes_1.StatusCodes.INTERNAL_SERVER_ERROR, `Error: exportImportService.replaceDuplicateIdsForCustomTemplate - ${(0, utils_1.getErrorMessage)(error)}`);
    }
}
async function replaceDuplicateIdsForDocumentStore(queryRunner, originalData, documentStores) {
    try {
        ...
        throw new internalTHubError_1.InternalTHubError(http_status_codes_1.StatusCodes.INTERNAL_SERVER_ERROR, `Error: exportImportService.replaceDuplicateIdsForDocumentStore - ${(0, utils_1.getErrorMessage)(error)}`);
    }
}
async function replaceDuplicateIdsForDocumentStoreFileChunk(queryRunner, originalData, documentStoreFileChunks) {
    try {
        ...
        throw new internalTHubError_1.InternalTHubError(http_status_codes_1.StatusCodes.INTERNAL_SERVER_ERROR, `Error: exportImportService.replaceDuplicateIdsForDocumentStoreFileChunk - ${(0, utils_1.getErrorMessage)(error)}`);
    }
}
async function replaceDuplicateIdsForTool(queryRunner, originalData, tools) {
    try {
        ...
        throw new internalTHubError_1.InternalTHubError(http_status_codes_1.StatusCodes.INTERNAL_SERVER_ERROR, `Error: exportImportService.replaceDuplicateIdsForTool - ${(0, utils_1.getErrorMessage)(error)}`);
    }
}
async function replaceDuplicateIdsForVariable(queryRunner, originalData, variables) {
    try {
        ...
        throw new internalTHubError_1.InternalTHubError(http_status_codes_1.StatusCodes.INTERNAL_SERVER_ERROR, `Error: exportImportService.replaceDuplicateIdsForVariable - ${(0, utils_1.getErrorMessage)(error)}`);
    }
}
async function replaceDuplicateIdsForExecution(queryRunner, originalData, executions) {
    try {
        ...
        throw new internalTHubError_1.InternalTHubError(http_status_codes_1.StatusCodes.INTERNAL_SERVER_ERROR, `Error: exportImportService.replaceDuplicateIdsForExecution - ${(0, utils_1.getErrorMessage)(error)}`);
    }
}
...
const importData = async (importData) => {
    ...
    try {
        ...
        try {
            ...
            if (importData.ChatMessage.length > 0)
                await queryRunner.manager.save(ChatMessage_1.ChatMessage, importData.ChatMessage);
            if (importData.ChatMessageFeedback.length > 0)
            ...
            if (importData.DocumentStoreFileChunk.length > 0)
                await queryRunner.manager.save(DocumentStoreFileChunk_1.DocumentStoreFileChunk, importData.DocumentStoreFileChunk);
            if (importData.Tool.length > 0)
        ...
        throw new internalTHubError_1.InternalTHubError(http_status_codes_1.StatusCodes.INTERNAL_SERVER_ERROR, `Error: exportImportService.importAll - ${(0, utils_1.getErrorMessage)(error)}`);
    }
};
exports.default = {
    convertExportInput,
