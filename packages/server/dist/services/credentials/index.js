...
const createCredential = async (requestBody) => {
    try {
        ...
        throw new internalTHubError_1.InternalTHubError(http_status_codes_1.StatusCodes.INTERNAL_SERVER_ERROR, `Error: credentialsService.createCredential - ${(0, utils_2.getErrorMessage)(error)}`);
    }
};
...
const deleteCredentials = async (credentialId) => {
    try {
        ...
        if (!dbResponse) {
            throw new internalTHubError_1.InternalTHubError(http_status_codes_1.StatusCodes.NOT_FOUND, `Credential ${credentialId} not found`);
        }
        return dbResponse;
    }
    catch (error) {
        throw new internalTHubError_1.InternalTHubError(http_status_codes_1.StatusCodes.INTERNAL_SERVER_ERROR, `Error: credentialsService.deleteCredential - ${(0, utils_2.getErrorMessage)(error)}`);
    }
};
const getAllCredentials = async (paramCredentialName, tenantId) => {
    try {
        const appServer = (0, getRunningExpressApp_1.getRunningExpressApp)();
        let dbResponse = [];
        if (paramCredentialName) {
            ...
            const credentials = await appServer.AppDataSource.getRepository(Credential_1.Credential).findBy({ tenantId });
        ...
        throw new internalTHubError_1.InternalTHubError(http_status_codes_1.StatusCodes.INTERNAL_SERVER_ERROR, `Error: credentialsService.getAllCredentials - ${(0, utils_2.getErrorMessage)(error)}`);
    }
};
...
const updateCredential = async (credentialId, requestBody) => {
    try {
        ...
        throw new internalTHubError_1.InternalTHubError(http_status_codes_1.StatusCodes.INTERNAL_SERVER_ERROR, `Error: credentialsService.updateCredential - ${(0, utils_2.getErrorMessage)(error)}`);
    }
};
exports.default = {
    createCredential,
