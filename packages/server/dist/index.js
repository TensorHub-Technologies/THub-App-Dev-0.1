...
const subscription_legacy_1 = __importDefault(require("./routes/subscription-legacy"));
const ensureUserSubscriptionColumns_1 = require("./database/utils/ensureUserSubscriptionColumns");
...
class App {
    ...
    async initDatabase() {
        ...
        try {
            ...
            await (0, ensureUserSubscriptionColumns_1.ensureUserSubscriptionColumns)(this.AppDataSource);
    ...
    async config() {
        ...
        this.app.use(express_1.default.json({ limit: thub_file_size_limit }));
        this.app.use(express_1.default.urlencoded({ limit: thub_file_size_limit, extended: true }));
        ...
        this.app.use('/', subscription_legacy_1.default);
    ...
    async stopApp() {
        try {
            ...
            logger_1.default.error(`❌[server]: THub Server shut down error: ${e}`);
        }
    }
}
exports.App = App;
...
async function start() {
    ...
    server.listen(port, host, () => {
        logger_1.default.info(`⚡️ [server]: THub Server is listening at ${host ? 'http://' + host : ''}:${port}`);
    });
}
