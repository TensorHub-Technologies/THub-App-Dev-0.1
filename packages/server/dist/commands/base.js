...
class BaseCommand extends core_1.Command {
    ...
    onTerminate() {
        return async () => {
            try {
                ...
                setTimeout(async () => {
                    logger_1.default.info('THub was forced to shut down after 30 secs');
                    await this.failExit();
                }, 30000);
                await this.stopProcess();
            }
            catch (error) {
                logger_1.default.error('There was an error shutting down THub...', error);
            }
        };
    }
    ...
    async init() {
        ...
        if (flags.THUB_FILE_SIZE_LIMIT)
            process.env.THUB_FILE_SIZE_LIMIT = flags.THUB_FILE_SIZE_LIMIT;
        // Credentials
        ...
        if (flags.THUB_SECRETKEY_OVERWRITE)
            process.env.THUB_SECRETKEY_OVERWRITE = flags.THUB_SECRETKEY_OVERWRITE;
...
BaseCommand.flags = {
    THUB_USERNAME: core_1.Flags.string(),
    THUB_PASSWORD: core_1.Flags.string(),
    THUB_FILE_SIZE_LIMIT: core_1.Flags.string(),
    ...
    THUB_SECRETKEY_OVERWRITE: core_1.Flags.string(),
