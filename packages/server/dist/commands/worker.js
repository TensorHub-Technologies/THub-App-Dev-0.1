...
class Worker extends base_1.BaseCommand {
    async run() {
        logger_1.default.info('Starting THub Worker...');
    ...
    async stopProcess() {
        try {
            ...
            logger_1.default.info(`Shutting down THub Prediction Worker ${this.predictionWorkerId}...`);
            await predictionWorker.close();
            const upsertWorker = queueManager.getQueue('upsert').getWorker();
            logger_1.default.info(`Shutting down THub Upsertion Worker ${this.upsertionWorkerId}...`);
            await upsertWorker.close();
        }
        catch (error) {
            logger_1.default.error('There was an error shutting down THub Worker...', error);
            await this.failExit();
        }
