 ...
 *
 * This module implements a complete OAuth2 authorization code flow for THub credentials.
...
router.post('/authorize/:credentialId', async (req, res, next) => {
    try {
        ...
        next(new internalTHubError_1.InternalTHubError(http_status_codes_1.StatusCodes.INTERNAL_SERVER_ERROR, `OAuth2 authorization error: ${error instanceof Error ? error.message : 'Unknown error'}`));
    }
});
...
router.post('/refresh/:credentialId', async (req, res, next) => {
    try {
        ...
        next(new internalTHubError_1.InternalTHubError(http_status_codes_1.StatusCodes.INTERNAL_SERVER_ERROR, `OAuth2 token refresh error: ${error instanceof Error ? error.message : 'Unknown error'}`));
    }
});
