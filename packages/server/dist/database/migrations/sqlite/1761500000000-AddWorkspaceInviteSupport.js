...
class AddWorkspaceInviteSupport1761500000000 {
    async up(queryRunner) {
        ...
        if (!(await queryRunner.hasTable('workspace_users'))) {
            ...
            `);
            await queryRunner.query('CREATE UNIQUE INDEX "idx_workspace_users_workspace_user_unique" ON "workspace_users" ("workspace_id", "user_id");');
        ...
        if (!(await queryRunner.hasTable('workspace_invites'))) {
            ...
            await queryRunner.query('CREATE INDEX "idx_workspace_invites_email_workspace" ON "workspace_invites" ("email", "workspace_id");');
        }
        if (!(await queryRunner.hasColumn('workspace_invites', 'created_at'))) {
            await queryRunner.query('ALTER TABLE "workspace_invites" ADD COLUMN "created_at" datetime NOT NULL DEFAULT (datetime(\'now\'));');
        }
