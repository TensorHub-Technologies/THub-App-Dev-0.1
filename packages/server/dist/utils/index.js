...
exports.REDACTED_CREDENTIAL_VALUE = '_THUB_BLANK_07167752-1a71-43b1-bf8f-4f32252165db';
...
const getEndingNodes = (nodeDependencies, graph, allNodes) => {
    ...
    for (const endingNode of endingNodes) {
        ...
        if (!endingNodeData) {
            error = new internalTHubError_1.InternalTHubError(http_status_codes_1.StatusCodes.INTERNAL_SERVER_ERROR, `Ending node ${endingNode.id} data not found`);
            continue;
        }
        ...
        if (!isEndingNode) {
            if (endingNodeData &&
                ...
                error = new internalTHubError_1.InternalTHubError(http_status_codes_1.StatusCodes.INTERNAL_SERVER_ERROR, `Ending node must be either a Chain or Agent or Engine`);
                continue;
            }
        }
    ...
    if (endingNodes.length === 0 || error === null) {
        error = new internalTHubError_1.InternalTHubError(http_status_codes_1.StatusCodes.INTERNAL_SERVER_ERROR, `Ending nodes not found`);
    }
    throw error;
};
...
const getVariableValue = async (paramValue, reactFlowNodes, question, chatHistory, isAcceptVariable = false, flowConfig, uploadedFilesContent, availableVariables = [], variableOverrides = []) => {
    ...
    while (startIdx < endIdx) {
        ...
        if (substr === '}}' && variableStack.length > 0 && variableStack[variableStack.length - 1].substr === '{{') {
             ...
             * FROM: hello i am ben\n\n\thow are you?
             * TO: hello i am benTHUB_NEWLINETHUB_NEWLINETHUB_TABhow are you?
             */
            if (isAcceptVariable && variableFullPath === exports.QUESTION_VAR_PREFIX) {
                variableDict[`{{${variableFullPath}}}`] = (0, thub_components_1.handleEscapeCharacters)(question, false);
            }
            if (isAcceptVariable && variableFullPath === exports.FILE_ATTACHMENT_PREFIX) {
                variableDict[`{{${variableFullPath}}}`] = (0, thub_components_1.handleEscapeCharacters)(uploadedFilesContent, false);
            }
            if (isAcceptVariable && variableFullPath === exports.CHAT_HISTORY_VAR_PREFIX) {
                variableDict[`{{${variableFullPath}}}`] = (0, thub_components_1.handleEscapeCharacters)((0, thub_components_1.convertChatHistoryToText)(chatHistory), false);
            }
            ...
            if (executedNode) {
                ...
                if (variableFullPathParts.length > 3) {
                    ...
                    switch (typeof variableValue) {
                        case 'string': {
                            const unEscapedVariableValue = (0, thub_components_1.handleEscapeCharacters)(variableValue, true);
                    ...
                    if (variableObj) {
                        ...
                        variableValue = (0, thub_components_1.handleEscapeCharacters)(typeof variableObj === 'object' ? JSON.stringify(variableObj) : variableObj, false);
                    }
                }
                if (isAcceptVariable) {
                    variableDict[`{{${variableFullPath}}}`] = variableValue;
...
const isStartNodeDependOnInput = (startingNodes, nodes) => {
    for (const node of startingNodes) {
        ...
        for (const inputName in node.data.inputs) {
            const inputVariables = (0, thub_components_1.getInputVariables)(node.data.inputs[inputName]);
...
const getEncryptionKey = async () => {
    ...
    try {
        return await fs_1.default.promises.readFile((0, thub_components_1.getEncryptionKeyPath)(), 'utf8');
    }
    catch (error) {
        const encryptKey = (0, exports.generateEncryptKey)();
        const defaultLocation = process.env.SECRETKEY_PATH
            ? path_1.default.join(process.env.SECRETKEY_PATH, 'encryption.key')
            : path_1.default.join((0, exports.getUserHome)(), '.thub', 'encryption.key');
...
const decryptCredentialData = async (encryptedData, componentCredentialName, componentCredentials) => {
    ...
    if (USE_AWS_SECRETS_MANAGER && secretsManagerClient) {
        try {
            if (encryptedData.startsWith('THubCredential_')) {
...
const getUserSettingsFilePath = () => {
    ...
    const checkPaths = [path_1.default.join((0, exports.getUserHome)(), '.thub', 'settings.json')];
    for (const checkPath of checkPaths) {
        if (fs_1.default.existsSync(checkPath)) {
            return checkPath;
        }
...
const getUploadPath = () => {
    return process.env.BLOB_STORAGE_PATH
        ...
        : path_1.default.join((0, exports.getUserHome)(), '.thub', 'uploads');
};
...
const getMulterStorage = () => {
    ...
    if (storageType === 's3') {
        const s3Client = (0, thub_components_1.getS3Config)().s3Client;
        const Bucket = (0, thub_components_1.getS3Config)().Bucket;
