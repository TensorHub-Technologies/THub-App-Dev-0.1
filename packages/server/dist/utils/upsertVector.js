...
const executeUpsert = async ({ componentNodes, incomingInput, chatflow, chatId, appDataSource, telemetry, cachePool, isInternal, files }) => {
    ...
    if (vsNodesWithFileUpload.length > 1) {
        throw new internalTHubError_1.InternalTHubError(http_status_codes_1.StatusCodes.INTERNAL_SERVER_ERROR, 'Multiple vector store nodes with fileUpload enabled');
    }
    ...
    if (vsNodes.length > 1 && !stopNodeId) {
        throw new internalTHubError_1.InternalTHubError(http_status_codes_1.StatusCodes.INTERNAL_SERVER_ERROR, 'There are multiple vector nodes, please provide stopNodeId in body request');
    }
    else if (vsNodes.length === 1 && !stopNodeId) {
        stopNodeId = vsNodes[0].data.id;
    }
    else if (!vsNodes.length && !stopNodeId) {
        throw new internalTHubError_1.InternalTHubError(http_status_codes_1.StatusCodes.NOT_FOUND, 'No vector node found');
    }
...
const upsertVector = async (req, isInternal = false) => {
    ...
    try {
        ...
        if (process.env.MODE === Interface_1.MODE.QUEUE) {
            ...
            appServer.metricsProvider?.incrementCounter(Interface_Metrics_1.THUB_METRIC_COUNTERS.VECTORSTORE_UPSERT, {
        ...
        appServer.metricsProvider?.incrementCounter(Interface_Metrics_1.THUB_METRIC_COUNTERS.VECTORSTORE_UPSERT, { status: Interface_Metrics_1.THUB_COUNTER_STATUS.FAILURE });
