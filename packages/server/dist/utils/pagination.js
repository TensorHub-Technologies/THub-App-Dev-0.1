...
const getPageAndLimitParams = (req) => {
    ...
    if (req.query.page) {
        ...
        if (page < 0) {
            throw new internalTHubError_1.InternalTHubError(http_status_codes_1.StatusCodes.PRECONDITION_FAILED, `Error: page cannot be negative!`);
        }
    }
    if (req.query.limit) {
        ...
        if (limit < 0) {
            throw new internalTHubError_1.InternalTHubError(http_status_codes_1.StatusCodes.PRECONDITION_FAILED, `Error: limit cannot be negative!`);
        }
    }
    return { page, limit };
};
