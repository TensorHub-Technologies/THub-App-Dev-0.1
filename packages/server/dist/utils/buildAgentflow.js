 ...
 */
const addExecution = async (appDataSource, agentflowId, agentFlowExecutedData, sessionId, tenantId) => {
 ...
 */
const updateExecution = async (appDataSource, executionId, data) => {
    ...
    var agentTokens = [];
    try {
        const exectutedDataList = JSON.parse(bodyExecution.executionData);
        let totalTime = 0;
        let totalTokens = 0;
        ...
        logger_1.default.error(`Error parsing execution data for execution ${executionId}: ${(0, utils_2.getErrorMessage)(err)}`);
    }
    ...
    const resolveNodeReference = async (value) => {
        ...
        for (const match of matches) {
            ...
            if (variableFullPath === _1.CHAT_HISTORY_VAR_PREFIX) {
                resolvedValue = resolvedValue.replace(match, (0, thub_components_1.convertChatHistoryToText)(chatHistory));
            }
 ...
 */
const executeNode = async ({ nodeId, reactFlowNode, nodes, edges, graph, reversedGraph, incomingInput, chatflow, chatId, sessionId, apiMessageId, evaluationRunId, parentExecutionId, pastChatHistory, prependedChatHistory, appDataSource, telemetry, componentNodes, cachePool, sseStreamer, baseURL, overrideConfig = {}, apiOverrideStatus = false, nodeOverrides = {}, variableOverrides = [], uploadedFilesContent = '', fileUploads, humanInput, agentFlowExecutedData = [], agentflowRuntime, abortController, parentTraceIds, analyticHandlers, isInternal, isRecursive, iterationContext }) => {
    try {
 ...
 */
const executeAgentFlow = async ({ componentNodes, incomingInput, chatflow, chatId, evaluationRunId, appDataSource, telemetry, cachePool, sseStreamer, baseURL, isInternal, uploadedFilesContent, fileUploads, signal: abortController, isRecursive = false, parentExecutionId, iterationContext, isTool = false, tenantId }) => {
    ...
    */
    ...
    try {
        if (chatflow.analytic) {
            ...
            analyticHandlers = thub_components_1.AnalyticHandler.getInstance({ inputs: { analytics: analyticInputs } }, {
                appDataSource,
                databaseEntities: _1.databaseEntities,
                componentNodes,
                analytic: chatflow.analytic,
