...
const generateTTSForResponseStream = async (responseText, textToSpeechConfig, options, chatId, chatMessageId, sseStreamer, abortController) => {
    try {
        ...
        if (!activeProviderConfig)
            return;
        await (0, thub_components_1.convertTextToSpeechStream)(responseText, activeProviderConfig, options, abortController || new AbortController(), (format) => {
...
const initEndingNode = async ({ endingNodeIds, componentNodes, reactFlowNodes, incomingInput, flowConfig, uploadedFilesContent, availableVariables, apiOverrideStatus, nodeOverrides, variableOverrides }) => {
    ...
    if (!nodeToExecute) {
        throw new internalTHubError_1.InternalTHubError(http_status_codes_1.StatusCodes.NOT_FOUND, `Node not found`);
    }
 ...
 */
const executeFlow = async ({ componentNodes, incomingInput, chatflow, chatId, isEvaluation, evaluationRunId, appDataSource, telemetry, cachePool, sseStreamer, baseURL, isInternal, files, signal, isTool, tenantId }) => {
    ...
    if (uploads) {
        ...
        for (let i = 0; i < fileUploads.length; i += 1) {
            ...
            if ((upload.type === 'file' || upload.type === 'file:rag' || upload.type === 'audio') && upload.data) {
                ...
                await (0, thub_components_1.addSingleFileToStorage)(mime, bf, filename, chatflowid, chatId);
            ...
            if (upload.mime === 'audio/webm' || upload.mime === 'audio/mp4' || upload.mime === 'audio/ogg') {
                ...
                if (speechToTextConfig) {
                    ...
                    const speechToTextResult = await (0, thub_components_1.convertSpeechToText)(upload, speechToTextConfig, options);
    ...
    if (files?.length) {
        ...
        for (const file of files) {
            ...
            const storagePath = await (0, thub_components_1.addArrayFilesToStorage)(file.mimetype, fileBuffer, file.originalname, fileNames, chatflowid);
    ...
    if (isAgentFlow) {
        ...
        if (streamResults) {
            ...
            if (agentflow.followUpPrompts) {
                ...
                const generatedFollowUpPrompts = await (0, thub_components_1.generateFollowUpPrompts)(followUpPromptsConfig, apiMessage.content, {
                    chatId,
                    chatflowid: agentflow.id,
                    appDataSource,
                    databaseEntities: _1.databaseEntities
            ...
            if (chatflowConfig?.postProcessing?.enabled === true) {
                try {
                    ...
                    if (typeof moderatedResponse === 'string') {
                        result.text = (0, thub_components_1.handleEscapeCharacters)(moderatedResponse, true);
                    }
        ...
        if (chatflow.followUpPrompts) {
            ...
            const followUpPrompts = await (0, thub_components_1.generateFollowUpPrompts)(followUpPromptsConfig, apiMessage.content, {
                chatId,
                chatflowid,
                appDataSource,
                databaseEntities: _1.databaseEntities
        ...
        if (evaluationRunId) {
            const metrics = await thub_components_1.EvaluationRunner.getAndDeleteMetrics(evaluationRunId);
...
const checkIfStreamValid = async (endingNodes, nodes, streaming) => {
    ...
    for (const endingNode of endingNodes) {
        ...
        if (endingNodeData.outputs &&
            ...
            throw new internalTHubError_1.InternalTHubError(http_status_codes_1.StatusCodes.INTERNAL_SERVER_ERROR, `Output of ${endingNodeData.label} (${endingNodeData.id}) must be ${endingNodeData.label}, can't be an Output Prediction`);
        }
...
const utilBuildChatflow = async (req, isInternal = false) => {
    ...
    if (!chatflow) {
        throw new internalTHubError_1.InternalTHubError(http_status_codes_1.StatusCodes.NOT_FOUND, `Chatflow ${chatflowid} not found`);
    }
    ...
    const isTool = req.get('thub-tool') === 'true';
    const isEvaluation = req.headers['X-THub-Evaluation'] || req.body.evaluation;
    let evaluationRunId = '';
    ...
    try {
        ...
        if (e instanceof internalTHubError_1.InternalTHubError && e.statusCode === http_status_codes_1.StatusCodes.UNAUTHORIZED) {
            throw e;
        }
        else {
            throw new internalTHubError_1.InternalTHubError(http_status_codes_1.StatusCodes.INTERNAL_SERVER_ERROR, (0, utils_2.getErrorMessage)(e));
        }
    }
};
...
const incrementSuccessMetricCounter = (metricsProvider, isInternal, isAgentFlow) => {
    if (isAgentFlow) {
        metricsProvider?.incrementCounter(isInternal ? Interface_Metrics_1.THUB_METRIC_COUNTERS.AGENTFLOW_PREDICTION_INTERNAL : Interface_Metrics_1.THUB_METRIC_COUNTERS.AGENTFLOW_PREDICTION_EXTERNAL, { status: Interface_Metrics_1.THUB_COUNTER_STATUS.SUCCESS });
    }
    else {
        metricsProvider?.incrementCounter(isInternal ? Interface_Metrics_1.THUB_METRIC_COUNTERS.CHATFLOW_PREDICTION_INTERNAL : Interface_Metrics_1.THUB_METRIC_COUNTERS.CHATFLOW_PREDICTION_EXTERNAL, { status: Interface_Metrics_1.THUB_COUNTER_STATUS.SUCCESS });
    }
};
/**
...
const incrementFailedMetricCounter = (metricsProvider, isInternal, isAgentFlow) => {
    if (isAgentFlow) {
        metricsProvider?.incrementCounter(isInternal ? Interface_Metrics_1.THUB_METRIC_COUNTERS.AGENTFLOW_PREDICTION_INTERNAL : Interface_Metrics_1.THUB_METRIC_COUNTERS.AGENTFLOW_PREDICTION_EXTERNAL, { status: Interface_Metrics_1.THUB_COUNTER_STATUS.FAILURE });
    }
    else {
        metricsProvider?.incrementCounter(isInternal ? Interface_Metrics_1.THUB_METRIC_COUNTERS.CHATFLOW_PREDICTION_INTERNAL : Interface_Metrics_1.THUB_METRIC_COUNTERS.CHATFLOW_PREDICTION_EXTERNAL, { status: Interface_Metrics_1.THUB_COUNTER_STATUS.FAILURE });
    }
};
