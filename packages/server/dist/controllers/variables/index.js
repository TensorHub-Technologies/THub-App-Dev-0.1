...
const createVariable = async (req, res, next) => {
    try {
        if (typeof req.body === 'undefined') {
            throw new internalTHubError_1.InternalTHubError(http_status_codes_1.StatusCodes.PRECONDITION_FAILED, `Error: variablesController.createVariable - body not provided!`);
        }
        const body = req.body;
...
const deleteVariable = async (req, res, next) => {
    try {
        if (typeof req.params === 'undefined' || !req.params.id) {
            throw new internalTHubError_1.InternalTHubError(http_status_codes_1.StatusCodes.PRECONDITION_FAILED, 'Error: variablesController.deleteVariable - id not provided!');
        }
...
const getAllVariables = async (req, res, next) => {
    try {
        ...
        const apiResponse = await variables_1.default.getAllVariables(tenantId);
        return res.json(apiResponse);
    }
    catch (error) {
        next(error);
...
const updateVariable = async (req, res, next) => {
    try {
        if (typeof req.params === 'undefined' || !req.params.id) {
            throw new internalTHubError_1.InternalTHubError(http_status_codes_1.StatusCodes.PRECONDITION_FAILED, 'Error: variablesController.updateVariable - id not provided!');
        }
        if (typeof req.body === 'undefined') {
            throw new internalTHubError_1.InternalTHubError(http_status_codes_1.StatusCodes.PRECONDITION_FAILED, 'Error: variablesController.updateVariable - body not provided!');
        }
