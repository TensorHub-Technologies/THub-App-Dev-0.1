...
//const movieAgent = readFileSync("./dist/movie_agent.prompt", s"utf8");

const toolAgentPrompt = ai.prompt('tool_agent')

// 1. Define your agent's logic as a AgentExecutor
export class ToolExecutor implements AgentExecutor {
    ...
    */
        ...
        if (messages.length === 0) {
            console.warn(`[ToolAgentExecutor] No valid text messages found in history for task ${taskId}.`)
        ...
        try {
            ...
            /*
      const response = await toolAgentPrompt(
...
*/
            ...
            if (this.cancelledTasks.has(taskId)) {
                console.log(`[ToolAgentExecutor] Request cancelled for task: ${taskId}`)
            ...
            console.log(`[ToolAgentExecutor] Task ${taskId} finished with state: ${finalA2AState}`)
        } catch (error: any) {
            console.error(`[ToolAgentExecutor] Error processing task ${taskId}:`, error)
