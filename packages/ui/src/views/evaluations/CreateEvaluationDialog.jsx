...
const CreateEvaluationDialog = ({ show, dialogProps, onCancel, onConfirm }) => {
    ...
    useEffect(() => {
        ...
        console.log('getAllAgentflowsApi.data:', getAllAgentflowsApi.data)
        console.log('getAllChatflowsApi.data:', getAllChatflowsApi.data)
        console.log('getAssistantsApi.data:', getAssistantsApi.data)
        ...
        if (getAllAgentflowsApi.data && getAllChatflowsApi.data) {
            try {
                ...
                console.log('agentFlows', agentFlows)
    ...
    const populateFlowNames = (response, type) => {
        let flowNames = []
        // Extract the data array from the paginated response
        const data = response?.data || []

        console.log(`populateFlowNames for ${type}, data:`, data)
    ...
    const populateAssistants = (response) => {
        let assistantNames = []

        // Handle null response
        if (!response) return assistantNames

        // Extract the data array from the paginated response
        const assistants = response?.data || response

        console.log('populateAssistants, data:', assistants)
        ...
        if (Array.isArray(assistants)) {
            for (let i = 0; i < assistants.length; i += 1) {
                ...
                try {
                    ...
                    console.error('Error parsing assistant details:', e, assistant)
                }
            }
        }
